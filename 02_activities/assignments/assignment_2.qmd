...
# Read the .fam file
fam_data <- fread("../data/gwa.A2.fam")
...
(ii) Read the `.bim` file. How many SNPs does the dataset contain? 306102

```{r}
# Read the .bim file
bim_data <- fread("../data/gwa.A2.bim")
...
```{bash}
plink2 --bfile ../data/gwa.qc.A2 \
    --extract ../data/gwa.qc.A2.pruned.prune.in \
    --king-cutoff 0.0884 \
...
# Read GWAS output (PLINK2 names it *.PHENO1.glm.linear)
gwas <- fread("../data/gwa.qc_unrelated.A2.gwas.PHENO1.glm.linear")
...
# Remove NA p-values (e.g. monomorphic or non-ADD test rows)
gwas_clean <- gwas[TEST == "ADD" & !is.na(P)]
...
manhattan(gwas_clean,
          ...
          suggestiveline = -log10(1e-5),   # blue dashed line
          genomewideline = -log10(5e-8),   # red dashed line
...
# Lambda near 1.0 indicates no systematic inflation; >1.05 may suggest population stratification
```
